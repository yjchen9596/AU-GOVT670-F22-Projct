...
car_crash |> 
  select(-"OBJECTID") -> car_crash

car_crash_details |> 
  select(-"OBJECTID") -> car_crash_details
...
```{r}
a1 |> 
  select(-c("LOCATIONERROR", "LASTUPDATEDATE","MPDLATITUDE","MPDLONGITUDE","STREETSEGID", "TODATE","EVENTID","MAR_ADDRESS", "ROADWAYSEGID", "MPDGEOX","MPDGEOY")) -> a2
...
```{r}
no_na_all_car_crashes_data |> 
  filter(MAJORINJURY == "Y") -> major_injured_cases
```
...
no_na_all_car_crashes_data |> 
  filter(year > 2010) -> cases.after2010
