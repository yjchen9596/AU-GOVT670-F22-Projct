...
cases.after2010 <- car_crash |> 
  ...
  mutate(TIME_PERIOD = case_when( TIME > 6 & TIME < 12 ~ "Morning",
   ...
   dplyr::filter(YEAR > 2010) |> 
    rename_all(toupper)  |> 
            filter(PERSONTYPE ==  "Driver" | PERSONTYPE ==  "Passenger" | PERSONTYPE ==  "Pedestrian" | PERSONTYPE == "Bicyclist") |>
  mutate(
    ...
    #                          "Streetcar" = "NotApplicable"),
         ...
         INVEHICLETYPE = dplyr::recode(INVEHICLETYPE, "0" = "Unknown",
                             "Other Vehicle" = "Other",
                             "None" = "None",
                             "Unknown" = "Unknown", "Other Small/light Truck" = "Truck",
                             "Pickup Truck" = "Truck",
                             ...
                             "Cargo Van"="Van",
                             "Motorhome/camper/rv (recreational Vehicle)" = "Van",
                             "Atv (all Terrain Vehicle)" = "ATV",
                             "Drugs/ Narcotics"   = "Unknown",
                             "Firearms" = "Unknown"),
                     WARD = as.character(WARD),
                     INVEHICLETYPE = as.character(INVEHICLETYPE),
         WARD = if_else(WARD %in% c("Null", "UNKNOWN"), "Unknown", WARD),
         SPEEDING_INVOLVED = if_else(SPEEDING_INVOLVED == 1, 0, 1),
                  FATAL = ifelse(FATAL == "Y", 1, 0),
         TIME = as.double(TIME),
         ...
         WARD = as.factor(WARD)) |> 
  filter(AGE < 90 & AGE> 0)|> 
  filter(!(PERSONTYPE == "Driver" & AGE <= 16),WARD != "Unknown") |>   
  dplyr::select(-c(PERSONID,MAR_ID, NEARESTINTSTREETNAME, NEARESTINTROUTEID, YCOORD, XCOORD, LONGITUDE,LATITUDE, ADDRESS, FROMDATE, REPORTDATE, X, Y, CCN,  MEASURE,OFFSET,VEHICLEID, DATE, YEAR, CRIMEID, ROUTEID, INTAPPROACHDIRECTION, LICENSEPLATESTATE,TICKETISSUED,"OBJECTID.X","OBJECTID.Y", FATAL_PEDESTRIAN,MAR_SCORE,FATALPASSENGER,MAJORINJURY, MINORINJURY,FATAL_BICYCLIST,FATAL_DRIVER, SPEEDING)) |> 
  dplyr::select(-contains("IMPAIRED"), -contains("UNKNOWN"), -contains("INJURIES")) |> 
  dplyr::select(FATAL, everything())

  
# colnames(cases.after2010)
unique(cases.after2010$FATAL)
  # unique(cases.after2010$PERSONTYPE)

### dealing overwhelmingly unequal distribution
# Divide the data based on the 'group' column
# cases.after2010 <- cases.after2010 %>%
#   group_by(SPEEDING_INVOLVED) %>%
#   sample_n(size=5865)

## NOT WORTH TO USE IT AT ALL
 # PERSONTYPE    n   percent
 #     Driver 9810 0.8363171
 #  NonDriver 1920 0.1636829
write_rds(cases.after2010, file = "cases.after2010.rds", compress = "gz") ## rds file size is smaller
```
